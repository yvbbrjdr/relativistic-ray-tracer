...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
  ...
  Vector3D pinHole(lerp(bl.x, -bl.x, x), lerp(bl.y, -bl.y, y), -1);
  double lensRadiusSqrtRndR = lensRadius * sqrt(rndR);
  Vector3D pLens(lensRadiusSqrtRndR * cos(rndTheta), lensRadiusSqrtRndR * sin(rndTheta), 0);
  Ray ret(pos + c2w * pLens, (c2w * (pinHole * focalDistance - pLens)).unit(), fClip);
  ret.min_t = nClip;
  return ret;
}
